...
export function useRoqPlatformFileCategoryContentType(
  args?: FileCategoryContentTypeQueryVariables,
  swrOptions?: SWRRequestOptions<FileCategoryContentTypeQuery>,
) {
  const roq = useRoqClient();
  const key = JSON.stringify(['useRoqPlatformFileCategoryContentType', args || {}]);
  return useSWR<FileCategoryContentTypeQuery, Error>(
    key,
    async () => {
      const result = await roq.roqPlatform.fileCategoryContentType(args);
      return result;
    },
    {
      ...swrOptions,
